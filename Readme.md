...
* 解释单例模式
```
单例模式（Singleton Pattern）是一种创建型设计模式，确保一个类只有一个实例，并提供一个全局访问点。以下是其核心要点：
定义
单例模式限制一个类只能创建一个对象实例，并且该实例可以被全局访问。通常用于需要控制资源访问或确保全局唯一性的场景，例如日志管理器、数据库连接池或配置管理。
实现方式
单例模式通常通过以下方式实现：

私有构造函数：防止外部通过new创建实例。
静态实例：类内部维护一个静态的唯一实例。
全局访问方法：提供一个静态方法（如getInstance()）返回该唯一实例。
```